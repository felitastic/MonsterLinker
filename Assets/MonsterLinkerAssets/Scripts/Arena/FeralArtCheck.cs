                ...
                foreach (FAChain chain5 in Chain5)
                {
                    if (curBAlist.SequenceEqual(chain5.ChainInputList))
                    {
                        ...
                        if (baeffectshandler.curPlayerRP >= RPCostSum)
                        {
                            ...
                            AttackList.Clear();
                            AttackList.Add(chain5.NeededFeralArts[0]);
                            AttackList.Add(chain5.NeededFeralArts[1]);
    ...
    startFAloop:
        while (FANo < LoadedFeralArts.Count)
        {
            ...
            if (BAsToDelete.Count == LoadedFeralArts[FANo].FeralArtInput.Count)
            {
                ...
                //TODO not working correctly if there are 6 slots and 2 3part FAs
                if (baeffectshandler.curPlayerRP >= LoadedFeralArts[FANo].RPCost + RPCostSum)
                {
                    ...
                    //TODO SFA check used, maybe delete
                    ...
                    if (AttackList.Count == 2 && inputbarhandler.maxBaseAttackInputSlots == 6)
                    {
                        BAsToDelete.Clear();
                        BAsToDelete.Add(0);
                        BAsToDelete.Add(1);
